...
function getInsideOutsidePaths(route, polygon) {
    ...
    for(var i = 0; i < route.length-1; i++) {
        if(!(pointInPolygon(route[i], polygon) && pointInPolygon(route[i+1], polygon))) { //both verticies outside
            paths.push([route[i], route[i+1], "outside", distanceBetweenTwoPoints(route[i], route[i+1])])
        }
        else {
            paths.push([route[i], route[i+1], "inside", distanceBetweenTwoPoints(route[i], route[i+1])]);
        }
    }
    return paths;
};
