...
function aggregatePath(path) {
    var type = path[0][2];
    var startPoint = path[0][0];
    var endPoint = path[path.length-1][1];
    var length = 0;
    for(var i = 0; i < path.length; i++) {
        length += path[i][3];
    }
    return [type, startPoint, endPoint, length];
}
...
function getPaths(segments) {
    ...
    var lastSegment = segments1[segments1.length-1]; //get the last segment of the last path (there is an "one-of" error somewhre but i was unable to find it so it is corrected this way)
...
function calculateLengthOfPaths(segments) {
    var totalLength = 0;
...
function createPathTable(table, paths) {
    ...
    for(var i = 0; i < paths.length; i++) {
        tableData.push(aggregatePath(paths[i]));
    }

    //fill the table with the paths
    tableData.sort((a, b) => a[3] - b[3]); //sort by length
